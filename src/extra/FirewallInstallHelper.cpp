...
//--------------------------------------------------------------------------------------
UINT __stdcall SetMSIFirewallProperties( MSIHANDLE hModule ) 
{
    ...
    if( szProductCode )
    {
        ...
        if( ERROR_SUCCESS == MsiGetProductInfoW( szProductCode, L"InstallLocation", szInstallDir, &dwBufferSize ) )
            bGotInstallDir = true;
    }
    if( !bGotInstallDir )
    ...
    if( szFriendlyName && szRelativePath && szInstallDir ) 
    {
        ...
        StringCchCopyW( strFullPath, 1024, szInstallDir );
        StringCchCatW( strFullPath, 1024, szRelativePath );
        MsiSetPropertyW( hModule, L"RollBackAddToFirewall", strFullPath );
        MsiSetPropertyW( hModule, L"RemoveFromExceptionListUsingMSI", strFullPath );
        ...
        StringCchCopyW( szAdd, 1024, strFullPath );
        StringCchCatW( szAdd, 1024, L"|" );
        StringCchCatW( szAdd, 1024, szFriendlyName );
        MsiSetPropertyW( hModule, L"AddToExceptionListUsingMSI", szAdd );
    }
...
//--------------------------------------------------------------------------------------
UINT __stdcall AddToExceptionListUsingMSI( MSIHANDLE hModule ) 
{
    HRESULT hr = E_FAIL;
    ...
    if( szCustomActionData )
    {
        ...
        {
            *pDelim = 0;
            StringCchCopyW( strFriendlyAppName, 256, pDelim + 1 );
        }      

        StringCchCopyW( strGameExeFullPath, MAX_PATH, szCustomActionData );
...
//      "C:\MyGame\Game.exe"
//--------------------------------------------------------------------------------------
UINT __stdcall RemoveFromExceptionListUsingMSI( MSIHANDLE hModule ) 
{
    HRESULT hr = E_FAIL;
    WCHAR* szCustomActionData = GetPropertyFromMSI(hModule, L"CustomActionData");
    if( szCustomActionData )
...
LPWSTR GetPropertyFromMSI( MSIHANDLE hMSI, LPCWSTR szPropName )
{
    ...
    UINT uErr = MsiGetPropertyW( hMSI, szPropName, L"", &dwSize );
    if( (ERROR_SUCCESS == uErr) || (ERROR_MORE_DATA == uErr) )
    {
        ...
        if( szValue )
        {
            uErr = MsiGetPropertyW( hMSI, szPropName, szValue, &dwSize );
