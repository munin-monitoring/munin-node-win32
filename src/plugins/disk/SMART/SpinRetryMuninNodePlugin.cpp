...
#include "StdAfx.h"
#include "SpinRetryMuninNodePlugin.h"

SpinRetryMuninNodePlugin::SpinRetryMuninNodePlugin()
{
...
int SpinRetryMuninNodePlugin::GetConfig(char *buffer, int len)
{
  int ret = 0;
  int index = 0;

  ret = _snprintf(buffer, len, "graph_title Spin Retry Count\n"
    "graph_args --base 1000 -l 0\n"
    "graph_vlabel Count\n"
    "graph_category S.M.A.R.T\n"
    "graph_info This graph shows the Spin Retry Count of the hard drives in the machine.\n");
  buffer += ret;
  len -= ret;
...
int SpinRetryMuninNodePlugin::GetValues(char *buffer, int len)
{ 
  int index = 0;
  int ret;    
  ...
  for (index = 0; index < g_SmartReader.m_ucDrivesWithInfo; index++) {
    ...
    ST_SMART_INFO *pSmartInfo = g_SmartReader.GetSMARTValue(pDriveInfo->m_ucDriveIndex, SMART_ATTRIB_SPIN_RETRY_COUNT);
    if (!pSmartInfo)
      continue;
