...
#include "StdAfx.h"
#include "SeekErrorMuninNodePlugin.h"

SeekErrorMuninNodePlugin::SeekErrorMuninNodePlugin()
{
...
int SeekErrorMuninNodePlugin::GetConfig(char *buffer, int len)
{
  int ret = 0;
  int index = 0;

  ret = _snprintf(buffer, len, "graph_title Seek Error Rate\n"
    "graph_args --base 1000 -l 0\n"
    "graph_vlabel Real Value\n"
    "graph_category S.M.A.R.T\n"
    "graph_info This graph shows the Seek Error Rate of the hard drives in the machine.\n");
  buffer += ret;
  len -= ret;
...
int SeekErrorMuninNodePlugin::GetValues(char *buffer, int len)
{ 
  int index = 0;
  int ret;    
  ...
  for (index = 0; index < g_SmartReader.m_ucDrivesWithInfo; index++) {
    ...
    ST_SMART_INFO *pSmartInfo = g_SmartReader.GetSMARTValue(pDriveInfo->m_ucDriveIndex, SMART_ATTRIB_SEEK_ERROR_RATE);
    if (!pSmartInfo)
      continue;
