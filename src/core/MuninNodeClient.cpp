...
{	
  ...
  while (!TestDestroy()) {
    ...
    if (strstr(buffer, "quit") == buffer) {
      ...
      ret = _snprintf(buffer, BUFFER_SIZE, "munin node on %s version: Munin Node for Windows %i.%i.%i.%i\n", hostname, ver.GetFileVersionMajor(), ver.GetFileVersionMinor(), ver.GetFileVersionBuild(), ver.GetFileVersionQFE());
      ret = SendLine(buffer);        
